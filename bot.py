...
# SVG -> Lottie JSON konvertatsiyasi
...
def parse_color(value: Optional[str], default="#000000") -> Optional[List[float]]:
...
def safe_parse_svg(svg_bytes: bytes) -> ET.Element:
    """Xavfsiz SVG parse qilish - turli encodinglarni qo'llab-quvvatlaydi"""
    # Avval UTF-8 da sinab ko'ramiz
    try:
        svg_str = svg_bytes.decode("utf-8")
    except UnicodeDecodeError:
        # UTF-16 da sinab ko'ramiz
        try:
            svg_str = svg_bytes.decode("utf-16")
        except UnicodeDecodeError:
            # Oxirgi variant: ignore qilamiz
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Telegram limitini tekshirish - xavfsizlik uchun
        if size_kb > 64:
            await msg.delete()
            await update.message.reply_text(
                f"❌ Fayl hajmi {size_kb:.1f} KB - Telegram limiti 64 KB.\n\n"
                f"⚠️ SVG faylni optimallashtirib qayta yuboring:\n"
                f"• Murakkab elementlarni soddalashtiring\n"
                f"• Ranglar sonini kamaytiring\n"
                f"• Path ma'lumotlarini optimallashtiring"
            )
            return
        ...
        await msg.edit_text(f"❌ SVG fayl formati noto'g'ri: {e}\n\nIltimos, to'g'ri SVG fayl yuboring.")
...
def main():
    if not BOT_TOKEN or BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        logger.error("❌ BOT_TOKEN topilmadi! Iltimos, tokenni o'rnating.")
        return
    ...
    # FIXED: faqat matnli xabarlarni va command bo'lmaganlarni qabul qiladi
