...
async def menu(msg: types.Message):
    ...
    if not cats:
        ...
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=c[0].upper(), callback_data=f"cat_{c[0]}")] for c in cats])
...
async def show_products(call: types.CallbackQuery):
    ...
    text = f"{cat.upper()}:\n\n" + "\n".join([f"{p[1]} - {p[2]:,} so'm" for p in prods])
...
async def add(call: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT INTO cart (user_id, product_id, quantity) VALUES (?, ?, 1) ON CONFLICT DO UPDATE SET quantity = quantity + 1", (uid, pid))
        await db.commit()
...
async def order_table(msg: types.Message, state: FSMContext):
    ...
    if not items:
        ...
        await state.clear()
        return
    total = sum(p[1] * p[2] for p in items)
    prods = ", ".join([f"{p[0]} x{p[2]}" for p in items])
...
async def add_prod_start(msg: types.Message, state: FSMContext):
    await msg.answer("Nomi:")
    await state.set_state(AddProduct.name)

@dp.message(AddProduct.name)
async def add_prod_name(msg: types.Message, state: FSMContext):
    await state.update_data(name=msg.text)
    await msg.answer("Narxi (son):")
    await state.set_state(AddProduct.price)

@dp.message(AddProduct.price)
async def add_prod_price(msg: types.Message, state: FSMContext):
...
async def add_prod_cat(msg: types.Message, state: FSMContext):
