...
# ===================== SOZLAMALAR =====================
BOT_TOKEN = "8784152224:AAFS3SGLjniq-TkzFI1MQ-kP2HDP7jwABcM"          # @BotFather dan oling
ADMIN_IDS = [7966897703]               # O'zingizning Telegram ID (@userinfobot)
WALLET_ADDRESS = "T9zugMYn2giVbUQPxiX4quKpDugTwKuxZW"   # USDT TRC20 manzilingiz
MIN_DEPOSIT = 5                      # Minimal depozit (USD)
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        f"🤖 *2X Crypto Bot*ga xush kelibsiz!\n\n"
        f"💡 Qanday ishlaydi:\n"
        f"• Istalgan miqdorda pul kiriting\n"
        f"• 12 soat kuting\n"
        f"• Pulingiz 2 barobarga ko'payadi! 🚀\n\n"
        f"📌 Minimal depozit: *${MIN_DEPOSIT}*",
        parse_mode="Markdown",
...
async def show_balance(message: types.Message):
    ...
    for dep in active:
        ...
        if remaining.total_seconds() > 0:
            ...
            active_text += f"\n• ${dep['amount']:.2f} → ${dep['amount']*2:.2f} | ⏳ {hours}s {minutes}m qoldi"
        else:
            active_text += f"\n• ${dep['amount']:.2f} → ${dep['amount']*2:.2f} | ✅ Tayyor"

    await message.answer(
        f"💼 *Sizning hisobingiz*\n\n"
        f"💰 Balans: *${user['balance']:.2f}*\n"
        f"📥 Jami kiritilgan: *${user['total_deposited']:.2f}*\n"
        f"📤 Jami yechilgan: *${user['total_withdrawn']:.2f}*\n"
...
async def deposit_start(message: types.Message, state: FSMContext):
    await message.answer(
        f"💳 *Depozit qilish*\n\n"
        f"📌 Minimal miqdor: *${MIN_DEPOSIT}*\n\n"
        f"Qancha dollar kiritmoqchisiz? (Faqat raqam)",
        parse_mode="Markdown"
    )
...
async def deposit_amount(message: types.Message, state: FSMContext):
    try:
        ...
        if amount < MIN_DEPOSIT:
            await message.answer(f"❌ Minimal miqdor ${MIN_DEPOSIT}! Qayta kiriting:")
            return

        await state.update_data(amount=amount)
        await message.answer(
            f"✅ Miqdor: *${amount:.2f}*\n\n"
            f"📤 Quyidagi manzilga *USDT (TRC20)* yuboring:\n"
            f"`{WALLET_ADDRESS}`\n\n"
...
async def deposit_txid(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Tasdiqlangandan so'ng *12 soat* ichida pulingiz 2x bo'ladi!",
        parse_mode="Markdown",
        reply_markup=main_menu()
    )
    await state.clear()
...
async def approve_deposit(callback: types.CallbackQuery):
    ...
    try:
        await bot.send_message(
            ...
            f"💰 Miqdor: *${deposit['amount']:.2f}*\n"
            f"🚀 12 soatdan keyin *${deposit['amount']*2:.2f}* olasiz!\n"
    ...
    asyncio.create_task(auto_payout(deposit['user_id'], deposit['amount'] * 2, dep_id))
...
async def auto_payout(user_id: int, amount: float, dep_id: int):
    ...
    try:
        await bot.send_message(
            user_id,
            f"🎊 *Tabriklaymiz! Pulingiz 2x bo'ldi!*\n\n"
            f"💰 Hisobingizga *${amount:.2f}* qo'shildi!\n"
...
async def show_history(message: types.Message):
    ...
    for h in history:
        ...
        text += f"{emoji} ${h['amount']:.2f} — {date_str} — {h['status']}\n"
...
async def withdraw_start(message: types.Message, state: FSMContext):
    ...
    if not user or user['balance'] < 10:
        ...
        await message.answer(
            f"❌ Minimal yechish miqdori *$10*.\n"
            f"Sizning balansigiz: *${bal:.2f}*",
            parse_mode="Markdown"
        )
        return
    ...
    await message.answer(
        f"💸 *Pul yechish*\n\n"
        f"💰 Mavjud balans: *${user['balance']:.2f}*\n\n"
        f"USDT (TRC20) wallet manzilingizni kiriting:",
        parse_mode="Markdown"
    )
...
async def withdraw_amount(message: types.Message, state: FSMContext):
    try:
        ...
        if amount > user['balance']:
            ...
            return
        if amount < 10:
            await message.answer("❌ Minimal yechish $10!")
            return
...
async def admin_panel(message: types.Message):
    ...
    await message.answer(
        ...
        f"💰 Jami depozit: *${stats['total_deposits']:.2f}*\n"
        f"📤 Jami 2x to'lovlar: *${stats['total_payouts']:.2f}*\n"
...
async def admin_users(message: types.Message):
    ...
    for u in users:
        text += f"• {u['full_name']} | 💰${u['balance']:.2f} | {u['created_at'][:10]}\n"
    await message.answer(text, parse_mode="Markdown")
...
async def info(message: types.Message):
    await message.answer(
        "ℹ️ *Bot haqida*\n\n"
        "🤖 2X Crypto Bot — kripto investitsiya platformasi\n\n"
        "📌 Qoidalar:\n"
        f"• Minimal depozit: ${MIN_DEPOSIT}\n"
        "• 12 soatdan keyin 2x qaytarish\n"
        "• USDT TRC20 qabul qilinadi\n"
        "• Referal bonus: 5%\n\n"
