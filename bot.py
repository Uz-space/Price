...
async def show_cart(message: types.Message):
    ...
    # Create inline keyboard with quantity controls
...
# ========== BUYURTMA (STOL RAQAMI BILAN TASDIQLASH) ==========
...
async def get_table_number(message: types.Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Ha, to'g'ri", callback_data="confirm_table"),
         InlineKeyboardButton(text="❌ Yo'q, qayta kiritish", callback_data="retry_table")]
    ])
    await message.answer(f"🪑 Stol raqami: {table_number}\nTo'g'rimi?", reply_markup=kb)
...
async def confirm_table(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"⚠️ Agar buyurtmani bekor qilmoqchi bo'lsangiz, /cancel_{order_id} buyrug'ini yuboring.",
...
# ========== FOYDALANUVCHI BUYURTMANI BEKOR QILISH ==========
...
async def cancel_order(message: types.Message):
    ...
    await send_to_channel(order_id, "cancelled")
...
async def user_paid(callback: types.CallbackQuery):
    ...
    # Admin paneliga tugmalar bilan xabar
...
async def admin_reject_payment(callback: types.CallbackQuery):
    ...
    await send_to_channel(order_id, "cancelled")
    await callback.message.edit_text(f"❌ Buyurtma #{order_id} to‘lovi rad etildi.")
    await callback.answer()
...
async def my_orders(message: types.Message):
    ...
    for oid, prods, total, status, table, date in orders:
        status_emoji = "⏳" if status == "toʻlov_kutilmoqda" else "✅" if status == "qabul_qilingan" else "🚚" if status == "yetkazilgan" else "❌"
        text += f"#{oid} | {status_emoji} {status} | Stol: {table}\n{prods}\nJami: {total} so'm\n{date}\n"
        if status == "toʻlov_kutilmoqda":
            text += f"⚠️ Bekor qilish: /cancel_{oid}\n"
        text += "\n"
...
async def stats(message: types.Message):
    ...
    await message.answer(f"📊 *Statistika*\n"
                        ...
                        f"⏳ Kutilayotgan: {pending}", parse_mode="Markdown")
