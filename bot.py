...
admin_menu = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="Qoshish"), KeyboardButton(text="Ochirish")], [KeyboardButton(text="Buyurtmalar"), KeyboardButton(text="Statistika")]], resize_keyboard=True)
...
async def start(msg: types.Message):
    ...
    if msg.from_user.id == ADMIN_ID:
        ...
        await msg.answer("AlphaHookah", reply_markup=user_menu)
...
async def menu(msg: types.Message):
    ...
    if not cats:
        await msg.answer("Mahsulot yoq")
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=c[0], callback_data=f"cat_{c[0]}")] for c in cats])
    await msg.answer("Kategoriya:", reply_markup=kb)
...
async def show_products(call: types.CallbackQuery):
    ...
    for p in prods:
        text += f"{p[1]} - {p[2]} so'm\n"
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"Qoshish {p[1]}", callback_data=f"add_{p[0]}")] for p in prods] + [[InlineKeyboardButton(text="Ortga", callback_data="back")]])
...
async def back(call: types.CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT DISTINCT category FROM products") as cur:
            cats = await cur.fetchall()
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=c[0], callback_data=f"cat_{c[0]}")] for c in cats])
    await call.message.edit_text("Kategoriya:", reply_markup=kb)
    await call.answer()
...
async def add(call: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await call.answer("Qoshildi", show_alert=True)
...
async def cart(msg: types.Message):
    ...
    if not items:
        await msg.answer("Savatcha bosh")
        return
    total = 0
    text = "Savatcha:\n"
    for it in items:
...
async def order_start(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer("Stol raqami:")
...
async def order_table(msg: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await bot.send_message(ADMIN_ID, f"Yangi #{oid}\n{msg.from_user.first_name}\nStol {table}\n{prods}\n{total} so'm")
...
@dp.message(lambda m: m.text == "Qoshish" and m.from_user.id == ADMIN_ID)
...
async def add_category(msg: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await msg.answer(f"{data['name']} qoshildi")
    await state.clear()

@dp.message(lambda m: m.text == "Ochirish" and m.from_user.id == ADMIN_ID)
...
async def stats(msg: types.Message):
    ...
    await msg.answer(f"Buyurtma: {total}\nDaromad: {revenue} so'm\nMahsulot: {prods}")
